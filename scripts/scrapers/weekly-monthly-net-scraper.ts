...
interface WeeklyMonthlyNetListing extends ListingSource {
  pricingPlans?: any[];
  tags?: string[];
  lat?: number;
  lng?: number;
...
export async function fetchWeeklyMonthlyNetListings(): Promise<WeeklyMonthlyNetListing[]> {
  const listings: WeeklyMonthlyNetListing[] = [];
  let browser: any = null;
  
  try {
    console.log('Launching browser for weekly-monthly.net...');
    ...
    const url = 'https://www.weekly-monthly.net/tokyo/';
    ...
    const listingUrls = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/room/"], a[href*="/detail/"]').forEach((link) => {
        ...
        if (href && !urls.includes(href)) {
          urls.push(href.startsWith('http') ? href : `https://www.weekly-monthly.net${href}`);
        }
      });
    ...
    for (let i = 0; i < Math.min(listingUrls.length, 10); i++) {
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          document.querySelectorAll('img').forEach((img) => {
            ...
            if (src && (src.includes('.jpg') || src.includes('.jpeg'))) {
              photos.push(src.startsWith('http') ? src : `https://www.weekly-monthly.net${src}`);
            }
          });
        ...
        if (data.price > 30000) {
          listings.push({
            externalId: `wmn-${url.match(/\d+/)?.[0] || Date.now()}`,
            sourceUrl: url,
    ...
    await browser.close();
    console.log(`\n✓ Scraped ${listings.length} listings from weekly-monthly.net`);
    return listings;
    
  } catch (error) {
    console.error('weekly-monthly.net scraper error:', error);
...
if (require.main === module) {
  fetchWeeklyMonthlyNetListings().then(console.log).catch(console.error);
}
